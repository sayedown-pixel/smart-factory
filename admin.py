...
def show_users(t):
    ...
    st.markdown("---")
    st.markdown(f"### {t.get('add_user_title', '➕ Add New User')}")
    ...
    with col1:
        new_username = st.text_input(t.get("add_user_username", "Username"), key="admin_new_user")
        new_password = st.text_input(t.get("add_user_password", "Password"), type="password", key="admin_new_pass")
        new_name = st.text_input(t.get("add_user_name", "Full Name"), key="admin_new_name")
    with col2:
        roles = [
            t.get("user_role_admin", "Admin"),
            t.get("user_role_supervisor", "Production Supervisor"),
            t.get("user_role_technician", "Maintenance Technician"),
            t.get("user_role_storekeeper", "Storekeeper"),
            t.get("user_role_quality", "Quality Controller")
        ]
        role_keys = ["admin", "supervisor", "technician", "storekeeper", "quality"]
        role_mapping = dict(zip(roles, role_keys))
        selected_role_label = st.selectbox(t.get("add_user_role", "Role"), options=roles, key="admin_new_role")
        user_company_id = factory_id if factory_id else (companies[0]['id'] if companies else None)
    
    if st.button(t.get("add_user_btn", "➕ Add User"), type="primary", use_container_width=True):
        if not new_username or not new_password or not new_name:
            st.error(t.get("add_user_validation", "⚠️ Please fill all required fields"))
        else:
            existing_users = db_manager.get_all_users()
            username_exists = any(u['username'] == new_username for u in existing_users)
            if username_exists:
                st.error(t.get("add_user_exists", "⚠️ Username already exists"))
            else:
                role_key = role_mapping.get(selected_role_label, "operator")
                success = db_manager.create_user_with_company(
                ...
                if success:
                    st.success(t.get("add_user_success", "✅ User {username} created successfully").format(username=new_username))
                    st.rerun()
                else:
                    st.error(t.get("add_user_failed", "❌ Failed to create user"))
