...
LANG = {
    ...
    "en": {
        ...
        "speed_label":  "Line Speed (units/shift)",
    }
}
...
if selected_menu == L["menu"][0]:
    ...
    with st.form("prod_form"):
        ...
        with c2:
            ...
            line_speed = st.number_input(L["speed_label"], min_value=1000, value=35000, step=1000,
                                         help="أدخل السرعة الفعلية للخط في هذه الوردية")
        ...
        if st.form_submit_button(L["save_btn"]):
            ...
            eff     = round((total_b / line_speed) * 100, 1) if line_speed else 0
            save_production({
                "type":          "Production",
                "line":          selected_line,
                "date":          str(p_date),
