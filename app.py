...
def main():
    ...
    if st.session_state.get('show_factory_wizard', False):
        ...
        if st.button(t.get("wizard_back_login", "🔙 العودة لشاشة الدخول"), key="back_login_main", use_container_width=True):
    ...
    # ======================================================================
    # Trial Status Banner & Read-Only Mode
    # ======================================================================
    read_only = st.session_state.get('read_only', False)
    if read_only:
        st.error(f"🔒 {t.get('trial_expired', 'Trial Period Expired')} - {t.get('trial_expired_msg', 'Read-only mode. Please contact support for renewal.')}")
        with st.expander(t.get("support_title", "📞 Technical Support"), expanded=False):
            ...
            """, unsafe_allow_html=True)
    elif not st.session_state.get('is_super_admin', False):
        trial_days = st.session_state.get('trial_days_left', 60)
        if trial_days <= 3:
            st.warning(t.get("trial_expire_soon", "⚠️ Trial period will expire in {days} days").replace("{days}", str(trial_days)))
        elif trial_days <= 7:
            st.info(t.get("trial_expire_soon", "⚠️ Trial period will expire in {days} days").replace("{days}", str(trial_days)))
    ...
    read_only = st.session_state.get('read_only', False)
    write_pages = ["📈 Production", "🔧 Maintenance", "📦 Raw Materials", "🏭 Finished Goods", "👥 Users", "⚙️ Settings"]
    
    if read_only and selected_page in write_pages:
        st.warning(t.get("trial_expired_msg", "Read-only mode: trial period has expired. Please contact technical support for renewal."))
    ...
    elif selected_page == "🔧 BOM Editor":
        if st.session_state.get('user_role') == "admin":
            from bom_editor import show_bom_editor
            show_bom_editor(t)
        else:
            st.warning(t.get("unauthorized", "⛔ Unauthorized access"))
