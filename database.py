...
# database.py - داخل class DatabaseManager
    ...
    def save_production(self, data: dict):
        ...
        try:
            ...
            factory_id = data.get('company_id') or data.get('factory_id') or self._get_current_factory_id_for_save()
...
# database.py - داخل class DatabaseManager
    ...
    def create_company(self, name_ar: str, name_en: str, code: str, email: str = None, phone: str = None, address: str = None) -> int:
        """إنشاء شركة/مصنع جديد مع فترة تجريبية 60 يوم"""
        session = None
        try:
            session = self.get_session()
            now = datetime.now()
    ...
    def get_all_companies_with_subscription(self) -> list:
        """الحصول على قائمة جميع الشركات مع معلومات الاشتراك (للمشرف العام)"""
        session = None
        try:
            session = self.get_session()
            ...
            logger.error(f"get_all_companies_with_subscription error: {e}")
            return []
        finally:
            if session:
                session.close()

    def check_company_trial_status(self, company_id: int) -> dict:
        """التحقق من حالة الفترة التجريبية لشركة"""
        session = None
        try:
            session = self.get_session()
            company = session.query(Company).filter(Company.id == company_id).first()
            if not company:
                return {'is_expired': True, 'days_left': 0, 'trial_end': None}
            now = datetime.now()
            trial_end = company.subscription_end
            if not trial_end:
                return {'is_expired': False, 'days_left': 60, 'trial_end': None}
            ...
            logger.error(f"check_company_trial_status error: {e}")
            return {'is_expired': True, 'days_left': 0, 'trial_end': None}
        finally:
            if session:
                session.close()

    def renew_company_subscription(self, company_id: int, days: int = 60) -> bool:
        """تجديد اشتراك شركة لمدة أيام محددة"""
        session = None
        try:
            session = self.get_session()
            company = session.query(Company).filter(Company.id == company_id).first()
            ...
            if company.subscription_end and now > company.subscription_end:
                ...
                new_end = company.subscription_end + timedelta(days=days) if company.subscription_end else now + timedelta(days=days)
            ...
            if session:
                session.rollback()
            logger.error(f"renew_company_subscription error: {e}")
            return False
        finally:
            if session:
                session.close()
    ...
    def get_line_product_speeds(self, company_id: int, line_name: str) -> dict:
        """الحصول على سرعات الإنتاج من product_line_speed"""
        session = None
        try:
            session = self.get_session()
            ...
            result = {}
            for s in speeds:
                product = session.query(Product).filter(Product.id == s.product_id).first()
                if product:
                    name = product.name_en or product.name_ar
                    result[name] = s.speed_bottles_per_hour
            return result
        except Exception as e:
            logger.error(f"get_line_product_speeds error: {e}")
            return {}
        finally:
            if session:
                session.close()
    ...
    def get_company_shift(self, company_id: int) -> dict:
        """الحصول على معلومات الوردية لشركة"""
        session = None
        try:
            session = self.get_session()
            ...
            logger.error(f"get_company_shift error: {e}")
            return None
        finally:
            if session:
                session.close()

    def get_product_bom_details(self, company_id: int, product_id: int = None) -> list:
        """الحصول على تفاصيل BOM للمنتجات في شركة"""
        session = None
        try:
            session = self.get_session()
            query = session.query(ProductBOMDetail).filter(
                ProductBOMDetail.company_id == company_id
            )
            if product_id:
                query = query.filter(ProductBOMDetail.product_id == product_id)
            details = query.all()
            result = []
            for d in details:
                result.append({
                    ...
                    'preforms_per_unit': d.preforms_per_unit,
                    'caps_per_unit': d.caps_per_unit,
                    'labels_per_unit': d.labels_per_unit,
                    'packaging_type': d.packaging_type,
                    'units_per_carton': d.units_per_carton,
                    'units_per_shrink': d.units_per_shrink,
                    'shrink_pieces_per_roll': d.shrink_pieces_per_roll,
                    'label_glue_grams_per_bottle': d.label_glue_grams_per_bottle,
                    'carton_glue_grams_per_carton': d.carton_glue_grams_per_carton,
                    'min_stock_units': d.min_stock_units,
                    'preform_material_id': d.preform_material_id,
                    'cap_material_id': d.cap_material_id,
                    'label_material_id': d.label_material_id,
                    'carton_material_id': d.carton_material_id,
                    'shrink_material_id': d.shrink_material_id,
                    'shrink_divider_material_id': d.shrink_divider_material_id,
                    'shrink_dividers_per_pallet': d.shrink_dividers_per_pallet,
                    'label_glue_material_id': d.label_glue_material_id,
                    'carton_glue_material_id': d.carton_glue_material_id,
                    'shrink_film_material_id': d.shrink_film_material_id,
                    'shrink_film_grams_per_pallet': d.shrink_film_grams_per_pallet,
                    'units_per_pallet': d.units_per_pallet,
                })
            return result
        except Exception as e:
            logger.error(f"get_product_bom_details error: {e}")
            return []
        finally:
            if session:
                session.close()
