...
def get_effective_config(factory_id=None, line_name=None):
    ...
    result = {}
    db_config = db_manager.get_factory_config(factory_id, line_name) if factory_id else {}
    ...
    for line, cfg in CONFIG.items():
        ...
        # Load speeds from product_line_speed table
        if factory_id:
            db_line_speeds = db_manager.get_line_product_speeds(factory_id, line)
            if db_line_speeds:
                line_cfg['speed'].update(db_line_speeds)
